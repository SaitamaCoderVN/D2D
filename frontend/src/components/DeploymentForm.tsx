...
export default function DeploymentForm({ onDeploymentCreated }: DeploymentFormProps) {
  ...
  // Load config and verify network on mount
  useEffect(() => {
    const loadConfig = async () => {
      try {
        console.log('🔧 Loading backend configuration...');
        const configData = await configApi.getTreasuryConfig();
        console.log('✅ Config loaded:', configData);
        setConfig(configData);
        
        // Check if backend is in Devnet mode
        if (configData.environment !== 'devnet') {
          const warning = '⚠️  Backend is not in Devnet mode. Please set SOLANA_ENV=devnet';
          console.warn(warning);
          setNetworkWarning(warning);
        } else {
          console.log('✅ Backend is in Devnet mode');
  ...
  const ensureSufficientBalance = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (balance < requiredBalance) {
        console.log('⚠️  Insufficient balance, requesting airdrop...');
        ...
        try {
          ...
          toast.success(`✅ Airdrop received! Balance: ${(newBalance / LAMPORTS_PER_SOL).toFixed(4)} SOL`, { 
            id: 'airdrop',
            duration: 3000 
          });
          ...
          toast.error('Airdrop failed. You may need to get SOL from a faucet.', { id: 'airdrop' });
          
          // Still return true if we have some balance
          return balance > 0.1 * LAMPORTS_PER_SOL;
        }
      }
